...
if ($method == 'fetch_po') {
	...
	$query = "SELECT id, po_num, parts_code, parts_name, description, supplier_code, quantity, shipping_mode, date_created, neopos_no, Status FROM pss_po_details WHERE Status = 'Open'";
		...
		foreach ($stmt->fetchALL() as $j) {
			$c++;
				...
				echo '<tr>';
					...
					echo '<td style="cursor:pointer;" class="modal-trigger" data-toggle="modal" data-target="#purchase_order_modal" onclick="get_po_details(&quot;'.$j['id'].'~!~'.$j['po_num'].'~!~'.$j['neopos_no'].'~!~'.$j['Status'].'&quot;)">'.$j['po_num'].'</td>';
					echo '<td>'.$j['Status'].'</td>';
...
if ($method ==  'load_po_details_modal') {
	...
	$query = "SELECT po_num, parts_code, parts_name, description, supplier_code, quantity FROM pss_po_details WHERE id = '$id' AND po_num = '$po_num'";
	$stmt = $conn->prepare($query);
	if ($stmt->execute()) {
		foreach ($stmt->fetchALL() as $j) {
			$c++;
